    ...
    */
    ...
    function ComputeCurve ()
    {
        ...
        if (pointList.length > 1)
        {
            ...
            for (var i = 0; i < n; i++)
            {
                sum_xy += xScale.invert (pointList[i][0]) * yScale.invert (pointList[i][1]);
                sum_x  += xScale.invert (pointList[i][0]);
                sum_y  += yScale.invert (pointList[i][1]);
                sum_xx += xScale.invert (pointList[i][0]) * xScale.invert (pointList[i][0]);

                // uncomment for testing...
                // console.log (i, sum_xy, sum_x, sum_y, sum_xx)
            }
            ...
            for (var i = xScale.domain()[0]; i <= xScale.domain()[1]; i+=0.5)
                locData.push ( { x: xScale(i), y: yScale (slope*i + intercept) } );
            ...
            // update the equation 
            d3.select ('#equation')
              .html ('Solution: (' + slope.toFixed(4) + ') x + (' + intercept.toFixed(4) + ')'); 

            var SSE = 0;
            var meanY = 0;
            for (var i = 0; i < pointList.length; i++)
            {
                yValue = slope*xScale.invert(pointList[i][0]) + intercept;
                SSE   += (yScale.invert(pointList[i][1]) - yValue) * (yScale.invert(pointList[i][1]) - yValue);
                meanY += yValue;
            }
            ...
            for (var i = 0; i < pointList.length; i++)
            {
                SST += (yScale.invert(pointList[i][1]) - meanY) * (yScale.invert(pointList[i][1]) - meanY);
            }
